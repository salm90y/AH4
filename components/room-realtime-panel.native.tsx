    ...
    <FlatList
      ...
      renderItem={({ item }) => <View style={styles.memberRow}>
        ...
        {currentRole === "host" && item.role !== "host" ? <View style={styles.memberActions}>
          <Pressable accessibilityLabel={item.role === "moderator" ? "إلغاء الإشراف" : "تعيين مشرف"} onPress={() => onChange(item, item.role === "moderator" ? "member" : "moderator", item.permissions)} style={({ pressed }) => [styles.memberAction, item.role === "moderator" && styles.memberActionActive, pressed && styles.toolPressed]}><MaterialIcons color="#FFFFFF" name={item.role === "moderator" ? "verified-user" : "admin-panel-settings"} size={17} /></Pressable>
          {(["control_source", "control_playback", "moderate_chat"] as RoomPermission[]).map((permission) => <Pressable accessibilityLabel={`تبديل صلاحية ${permissionLabels[permission]}`} key={permission} onPress={() => onChange(item, "member", item.permissions.includes(permission) ? item.permissions.filter((value) => value !== permission) : [...item.permissions, permission])} style={({ pressed }) => [styles.permissionDot, item.permissions.includes(permission) && styles.permissionDotActive, pressed && styles.toolPressed]}><Text style={styles.permissionDotText}>{permissionLabels[permission].slice(0, 1)}</Text></Pressable>)}
        </View> : null}
      </View>}
